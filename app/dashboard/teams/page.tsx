...
import TeamsClient from '@/components/dashboard/teams/TeamsClient'
...
export default async function TeamsPage() {
  ...
  const { data: teams } = await admin.from('teams').select('*').eq('club_id', club.id).order('created_at', { ascending: false })
  const { data: members } = await admin.from('club_memberships').select('*, profiles(*), club_roles(*)').eq('club_id', club.id).eq('status', 'active')
  return <TeamsClient club={club} teams={teams || []} members={members || []} />
}
